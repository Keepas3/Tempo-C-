...
inline std::string master_pgn_path(const std::string& pgn_date, const std::string& games_base_dir = "games") {
    int year = 0, month = 0, day = 0;
    if (sscanf(pgn_date.c_str(), "%d.%d.%d", &year, &month, &day) >= 2 &&
        year > 0 && month >= 1 && month <= 12) {
        char buf[16];
        snprintf(buf, sizeof(buf), "%02d-%04d", month, year);
        return games_base_dir + "/" + std::to_string(year) + "/" + std::string(buf) + "_games.pgn";
    }
    return games_base_dir + "/unknown/unknown_games.pgn";
}
...
// imported from.
inline void append_to_master_file(const Game& g, const std::string& games_base_dir = "games") {
    std::string path = master_pgn_path(g.date, games_base_dir);
