...
// Auto-generated from lichess-org/chess-openings (a.tsv..e.tsv, MIT licensed).
// For each ECO code, the name of its shortest (most general) line is kept as
// a readable family name (e.g. C45 -> "Scotch Game"). Used as a fallback when
// a PGN has an ECO tag but no Opening tag (common in chess.com exports).
struct EcoEntry { const char* eco; const char* name; };

const EcoEntry ECO_TABLE[500] = {
    {"A00", "Amar Opening"},
...
// Looks up the family name for an ECO code (exact 3-character match, e.g.
// "C45"). Returns an empty string if not found.
inline std::string lookup_eco_name(const std::string& eco) {
    if (eco.size() != 3) return "";
    auto it = std::lower_bound(
        std::begin(ECO_TABLE), std::end(ECO_TABLE), eco,
        [](const EcoEntry& entry, const std::string& key) { return std::strcmp(entry.eco, key.c_str()) < 0; });
    if (it != std::end(ECO_TABLE) && eco == it->eco) return it->name;
    return "";
}
